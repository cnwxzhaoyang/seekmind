...
impl SearchIndex {
    ...
    pub fn index_document(
        ...
        self.reader.reload().map_err(|error| error.to_string())?;
        eprintln!(
            "[SeekMind] Tantivy indexed document path={} chunks={} total_docs={}",
            document.path,
            chunks.len(),
            self.doc_count()
        );
...
pub fn fulltext_index_dir() -> PathBuf {
    if let Ok(path) = std::env::var("SeekMind_TANTIVY_DIR") {
...
fn default_fulltext_index_dir() -> PathBuf {
    ...
    base.join("com.zhaoyang.SeekMind.dev").join("tantivy")
}
...
fn default_fulltext_index_dir() -> PathBuf {
    let base = cache_dir()
        .or_else(data_dir)
        .unwrap_or_else(|| PathBuf::from("."));
    base.join("com.zhaoyang.seekmind").join("tantivy")
}
