...
use crate::seekmind::sidecar::apply_network_proxy_environment;
use crate::seekmind::storage::db::Database;
...
fn build_answer_view(
    ...
    answer: String,
    state: String,
    sources: Vec<crate::seekmind::models::QaSourceView>,
    retrieval: crate::seekmind::models::QaRetrievalView,
    model: String,
    created_at: String,
    error: Option<String>,
...
pub async fn test_qa_connection(
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        ...
        eprintln!(
            "[SeekMind] testing qa model connection provider={} model={} base_url={}",
        ...
        let prompt = "You are a connection test for SeekMind. Reply with a short confirmation only.";
        ...
        if message.is_empty() && is_ollama {
            ...
            eprintln!(
                "[SeekMind] qa connection empty on openai-compatible endpoint, retrying ollama native api"
            );
...
pub async fn ask_question(
    ...
    let emit_progress = |payload: QaAnswerProgressView| {
        let _ = app.emit("seekmind:qa:answer-progress", payload);
    };
    ...
    tauri::async_runtime::spawn(async move {
        let task = Abortable::new(
            async move {
                ...
                eprintln!(
                    "[SeekMind] routing qa request {} to python sidecar, scope_paths={}, session_context_bytes={}",
                    job_id_for_task,
                ...
                let response = match python_client_for_task.ask_question_stream(&request, |event| {
                    ...
                    Err(error) => {
                        ...
                        eprintln!(
                            "[SeekMind] qa request {} failed in python sidecar: {}",
                ...
                if let Some(ref warning_message) = warning {
                    eprintln!(
                        "[SeekMind] qa request {} warning from python sidecar: {}",
