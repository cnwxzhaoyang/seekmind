 ...
 * @Description macOS Vision OCR helper and bundled runtime probe for DocMind.
 */
...
pub fn default_vision_ocr_languages() -> Vec<String> {
    let configured = std::env::var("DOCMIND_VISION_OCR_LANGS")
        .ok()
...
mod macos {
    ...
    fn extract_lines_from_observations(observations: &NSArray<NSObject>) -> Vec<String> {
        ...
        for index in 0..observations.len() {
            ...
            eprintln!(
                "[DocMind][VisionOCR] observation={} confidence={:.3} text={}",
                index + 1,
                confidence,
