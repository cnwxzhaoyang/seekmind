    ...
    >
      <template v-for="(item, itemIndex) in items" :key="item.key">
        ...
        <button
          ...
          class="seekmind-context-menu-item flex w-full items-center gap-2.5 rounded-md px-3 py-1.5 text-left text-xs transition focus-visible:outline-none"
          :class="[
            item.danger
              ? activeIndex === itemIndex
                ? 'seekmind-context-menu-item-danger-active text-danger'
              ...
              : activeIndex === itemIndex
                ? 'seekmind-context-menu-item-active text-primary'
