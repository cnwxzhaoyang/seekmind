...
const debugClick = (event: MouseEvent | KeyboardEvent) => {
  ...
  console.debug("[SeekMindSearchResultCard] row click", {
    id: props.item.id,
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-start justify-between gap-2">
          <div class="truncate text-[13px] font-semibold" :class="props.nested ? 'text-primary' : 'text-accent-text'">
            <SeekMindHighlightedText :text="item.file_name" :query="props.query" />
          </div>
        ...
        <div v-if="item.title_path || item.heading" class="mt-1 text-[11px] text-dim">
          {{ t("page.appSearch.detail.titlePath") }}：<SeekMindHighlightedText :text="item.title_path || item.heading" :query="props.query" />
        </div>
        <div class="mt-2 text-sm leading-6" :class="props.nested ? 'text-secondary' : 'text-secondary'">
          <SeekMindHighlightedText :text="item.snippet" :query="props.query" :spans="item.highlight_spans" />
        </div>
        <div class="mt-2 flex flex-wrap items-center gap-1.5 text-[11px] text-dim">
          <SeekMindBadge>{{ props.nested ? "片段" : t("page.appSearch.detail.titlePath") }}</SeekMindBadge>
