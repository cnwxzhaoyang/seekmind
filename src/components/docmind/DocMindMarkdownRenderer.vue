...
const decorateCitationTokens = async () => {
  ...
  while (walker.nextNode()) {
    ...
    if (!parent || parent.closest("pre, code, .docmind-citation-chip")) {
      continue;
    }
  ...
  textNodes.forEach((node) => {
    ...
    for (const match of matches) {
      ...
      if (citationSourceIdSet.value.has(sourceId)) {
        ...
        button.type = "button";
        button.className =
          "docmind-citation-chip inline-flex items-center rounded-full border border-accent/20 bg-accent-soft px-2 py-0.5 text-[11px] font-medium text-accent transition hover:bg-accent-soft/80";
...
<template>
  <div class="docmind-markdown-renderer markdown-body" ref="contentRef">
