# SeekMind Python 侧 RAG Graph 设计

## 1. 结论

SeekMind 的问答能力要从“能召回、能回答”升级到“能稳定根据证据组织答案”，核心不在继续堆 prompt，而在把问答编排显式化为一个有状态的 RAG Graph。
...
对 SeekMind 来说，RAG Graph 应完全放在 Python sidecar 侧，Rust/Tauri 只负责：
...
这也符合 SeekMind 现有的 Python sidecar 任务模式：Rust 发起任务，Python 执行任务并持续输出 event，Rust 消费 event 并驱动 UI。
