# SeekMind Python 解析架构设计

本文定义 SeekMind 的 Python 解析 Sidecar 协议、数据结构、错误码和超时机制。目标是把文档解析和切片能力从 Rust 主进程中拆出去，先由 Python 负责 `md/docx/html/txt` 的解析与 chunk 生成，Rust 继续负责扫描、SQLite、Tantivy、搜索和 UI。
...
SeekMind 默认按“单次调用、单次返回”的方式使用 Python 解析器：
...
- 环境变量：`SeekMind_PARSER_TIMEOUT_MS`
- 默认值：`30000`
- 语义：单次文件解析允许的最长时间
- 超时后：
...
- `SeekMind_USE_PY_PARSER`
  - `1` 表示启用 Python 解析器
  - 未设置或 `0` 表示继续使用 Rust fallback
- `SeekMind_PARSER_BIN`
  - Python 可执行文件，默认 `python3`
- `SeekMind_PARSER_SCRIPT`
  - 入口脚本路径，默认 `parser/SeekMind_parser/__main__.py`
- `SeekMind_PARSER_TIMEOUT_MS`
  - 单次解析超时时间，默认 `30000`
- `SeekMind_TRACE_INDEXER`
  - 输出索引任务日志到终端，便于排查
