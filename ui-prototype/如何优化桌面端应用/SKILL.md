---
name: imagegen
description: "Use for visual deliverable routing and image generation/editing tasks, including new images, image edits, UI mockups, diagrams, infographics, posters, icons, logos, charts, visual assets, and ambiguous visual requests. Helps choose between Mermaid, Python plotting, static layout/code screenshots, web/app development, and AI image generation/editing."
...
| Existing image plus requests to replace, remove, translate, localize, restyle, add objects, alter a region, preserve identity/product, or change content | Use image editing/generation. Preserve unchanged areas explicitly. Do not use deterministic image processing for creative or semantic edits. |
| Pure deterministic manipulation such as resize, crop, rotate, flip, format conversion, simple stitching, or compression without content change | Use deterministic image processing. |
...
Use deterministic image processing only for explicit user-requested operations such as resize, crop, format conversion, compression, or background removal, or when a required delivery format/size is clearly impossible without it. Never use code-based cropping, local preview extraction, font search, or scripted patching merely as a self-check after AI generation.
